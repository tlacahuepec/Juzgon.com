...
fun Category.toAttributeEntities(): List<AttributeEntity> = attributes.map { attributeId ->
    AttributeEntity(id = attributeId, categoryName = name)
}

fun CategoryEntity.toDomain(attributes: List<AttributeEntity>): Category =
    Category(name = name, attributes = attributes.map { it.id })

fun RatedItem.toItemEntity(): ItemEntity = ItemEntity(id = id)

fun RatedItem.toRatingEntities(): List<RatingEntity> = scores.map { scoreEntry ->
    RatingEntity(itemId = id, attributeId = scoreEntry.attribute.id, score = scoreEntry.score)
}

fun ItemEntity.toDomain(ratings: List<RatingEntity>, attributesById: Map<String, Attribute>): RatedItem {
    val scoreEntries = ratings.map { rating ->
        val attribute = checkNotNull(attributesById[rating.attributeId]) {
